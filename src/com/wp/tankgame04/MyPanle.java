...
public class MyPanle extends JPanel implements KeyListener, Runnable {
    ...
    public MyPanle(String key){
        if(key.equals("2")){
            String path = Recorder.getPathName();
            File file = new File(path);
            if(!file.exists()){
                System.out.println("上一把的记录不存在，只能开启新游戏");
                key = "1";
            }else{
                try {
    ...
    public void showGrade(Graphics g){
        g.setColor(Color.BLACK);
        Font font = new Font("宋体",Font.BOLD,25);
        g.setFont(font);
        g.drawString("您累计击毁坦克" + Recorder.getAlltankNums(),1020,30);
        drawTank(1020,60,g,0,0);
        g.setColor(Color.BLACK);
        g.drawString("" + Recorder.getAlltankNums(),1080,100);
    }
    @Override
    public void paint(Graphics g) {
        super.paint(g);
        g.fillRect(0, 0, 1020, 750);
        showGrade(g);
        if(hero.isLive){
